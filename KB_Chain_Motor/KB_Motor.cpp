...
void KB_MOTOR::begin(uint8_t address1, uint8_t address2) {
  ...
  /* Store the I2C address and init the Wire1 library */
  _address1 = address1;
  _address2 = address2;
  Wire1.begin(4, 5); // join another i2c bus edit KB_Relay.h default for i2c KidBright32

  // WriteCommand(0x00, 0x00);

  Wire1.beginTransmission(_address1);
  Wire1.write(0x01);
  Wire1.write(0b100000000);
  Wire1.endTransmission();

  Wire1.beginTransmission(_address2);
  Wire1.write(0x01);
  Wire1.write(0b100000000);
  Wire1.endTransmission();
}
...
void KB_MOTOR::faultCheck1(int _address1) {
  ...
  Wire1.beginTransmission(_addr1);
  Wire1.write(0x01);
  Wire1.endTransmission();

  Wire1.requestFrom(_addr1, 1);
  byte error = Wire1.read();
  //  printf(error);
  //  printf("\n");
  if (error & 0b00000011) {
...
void KB_MOTOR::faultCheck2(int _address2) {
  ...
  Wire1.beginTransmission(_addr2);
  Wire1.write(0x01);
  Wire1.endTransmission();

  Wire1.requestFrom(_addr2, 1);
  byte error = Wire1.read();
  if (error & 0b00000011) {
